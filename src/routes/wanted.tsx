...
import { timeAgo, useWanted, type WantedRequest } from "@/lib/wanted";

export const Route = createFileRoute("/wanted")({
  head: () => ({
    meta: [
      { title: "Wanted Board — Post What You Need | Meremoth Mall" },
      {
        name: "description",
        content:
          "Can't find it? Post a WANTED request on Meremoth Mall and let Douala vendors send you offers with escrow protection.",
      },
      { property: "og:title", content: "Wanted Board — Meremoth Mall" },
      {
...
function WantedPage() {
  const { items, add } = useWanted();

  return (
    <MallShell title="Wanted Board" back>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl font-bold">Wanted Board</h1>
          <p className="text-xs text-muted-foreground">
            Post what you need — Douala vendors send you offers. Escrow applies once you agree.
          </p>
        </div>
        <PostWantedDialog onSubmit={add} />
      </div>

      <div className="mt-4 space-y-3">
        {items.map((w) => (
          <WantedCard key={w.id} req={w} />
        ))}
      </div>
    </MallShell>
  );
...
function WantedCard({ req }: { req: WantedRequest }) {
...
export function PostWantedDialog({
  onSubmit,
  trigger,
}: {
  onSubmit: (req: {
      ...
      <DialogTrigger asChild>
        {trigger ?? (
          <Button className="shrink-0 rounded-2xl bg-amber-500 text-white hover:bg-amber-600">
            <Megaphone className="mr-1 h-4 w-4" /> Post WANTED
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Post a WANTED request</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1.5">
          ...
          <Button
            ...
            onClick={() => {
              ...
              toast.success("WANTED request posted — vendors can now send offers");
              setOpen(false);
              setProduct("");
              setDescription("");
              setBudget("");
